...
function updateAdminUI() {
    ...
    if (pcatEl) {
        ...
        Object.keys(catGroups).forEach(function(grp) {
            var grpCats = catGroups[grp].filter(function(c){ return db.categories.includes(c); });
            if (grpCats.length) {
                html += '<optgroup label="── ' + grp + ' ──">';
                grpCats.forEach(function(c){ usedCats.add(c); html += '<option value="'+sanitize(c)+'">'+sanitize(typeof tCat==='function'?tCat(c):c)+'</option>'; });
                html += '</optgroup>';
            }
        });
        // Any ungrouped cats
        var others = db.categories.filter(function(c){ return !usedCats.has(c); });
        if (others.length) {
    ...
    // Populate categories management table
    var catTable = document.getElementById('cat-table');
    if (catTable) {
        if (!db.categories || !db.categories.length) {
            catTable.innerHTML = '<tr><td colspan="3" style="text-align:center;padding:20px;color:var(--adm-muted,#6b7a99);">لا توجد أقسام — أضف قسماً أعلاه</td></tr>';
        } else {
            catTable.innerHTML = db.categories.map(function(cat, i) {
                var label = typeof tCat === 'function' ? tCat(cat) : cat;
                return '<tr>' +
                    '<td style="font-weight:700;">' + sanitize(label) + '</td>' +
                    '<td style="color:var(--adm-muted,#6b7a99);font-size:.8rem;">' + sanitize(cat) + '</td>' +
                    '<td style="text-align:left;">' +
                        '<button class="btn-del" onclick="deleteCategory(' + i + ')" title="حذف"><i class="fas fa-trash"></i></button>' +
                    '</td>' +
                '</tr>';
            }).join('');
        }
    ...
    document.getElementById('set-instapay').value = db.payment?.instapay   || '';

    document.getElementById('set-vodafone').value = db.payment?.vodafone   || '';

    // Hero tab
    const h = db.settings.hero || {};
    const htEl = document.querySelector(`input[name="hero-type"][value="${h.type||'text'}"]`);
    if (htEl) htEl.checked = true;
    document.getElementById('hero-title-input').value = h.title    || 'MARASSiA';
    document.getElementById('hero-sub-input').value   = h.subtitle || '';
    document.getElementById('hero-media-url').value   = h.media    || '';
    heroAdminPreview();

    // Social tab
    document.getElementById('set-fb').value  = db.social.fb  || '';
    document.getElementById('set-yt').value  = db.social.yt  || '';
    document.getElementById('set-ig').value  = db.social.ig  || '';
    document.getElementById('set-tt').value  = db.social.tt  || '';
    document.getElementById('set-pin').value = db.social.pin || '';
    updateSocialPreview();

    // About tab
    document.getElementById('set-about-text').value = db.settings.aboutUs || '';

    // Products table (FIX #7 — only this table, not full re-render)
    var trLangs = ['ar','en','fr','es','it','de','zh'];
    document.getElementById('prod-table').innerHTML =
        '<tr><th>المنتج</th><th>السعر</th><th>ترجمة</th><th>حذف</th></tr>' +
        db.products.map(function(p) {
            var trCount = p.translations ? Object.keys(p.translations).length : 0;
            var trBadge = trCount >= 7
                ? '<span style="background:#22c55e;color:#fff;border-radius:5px;padding:2px 7px;font-size:.68rem;">🌍 7</span>'
                : trCount > 0
                    ? '<span style="background:#f59e0b;color:#fff;border-radius:5px;padding:2px 7px;font-size:.68rem;">🔄 ' + trCount + '/7</span>'
                    : '<span style="background:#e5e7eb;color:#666;border-radius:5px;padding:2px 7px;font-size:.68rem;">—</span>';
            return '<tr>'
                + '<td style="font-weight:800;text-align:right;max-width:140px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">' + sanitize(p.name) + '</td>'
                + '<td style="color:var(--primary-blue);font-weight:800;">' + Number(p.price).toLocaleString() + '</td>'
                + '<td style="white-space:nowrap;">' + trBadge
                    + ' <button onclick="retranslateProduct(' + p.id + ')" style="background:var(--primary-blue);color:#fff;border:none;border-radius:5px;padding:2px 8px;cursor:pointer;font-size:.68rem;">↺</button></td>'
                + '<td style="white-space:nowrap;">'
                    + '<button onclick="editProduct(' + p.id + ')" style="background:#f59e0b;color:#fff;border:none;border-radius:5px;padding:3px 9px;cursor:pointer;font-size:.72rem;margin-left:4px;"><i class="fas fa-edit"></i></button>'
    ...
    document.getElementById('blog-table').innerHTML =
        `<tr><th>العنوان</th><th>الستايل</th><th>حذف</th></tr>` +
        db.blogs.map(function(b) {
            var trCount = b.translations ? Object.keys(b.translations).length : 0;
            var trBadge = trCount >= 7
                ? '<span style="background:#22c55e;color:#fff;border-radius:5px;padding:2px 7px;font-size:.68rem;">🌍 7</span>'
                : trCount > 0
                    ? '<span style="background:#f59e0b;color:#fff;border-radius:5px;padding:2px 7px;font-size:.68rem;">🔄 ' + trCount + '/7</span>'
                    : '<span style="background:#e5e7eb;color:#666;border-radius:5px;padding:2px 7px;font-size:.68rem;">—</span>';
            return '<tr>'
                + '<td style="font-weight:800;text-align:right;">' + sanitize(b.title) + '</td>'
                + '<td>' + trBadge + ' <button onclick="retranslateBlog(' + b.id + ')" style="background:var(--primary-blue);color:#fff;border:none;border-radius:5px;padding:2px 8px;cursor:pointer;font-size:.68rem;">↺</button></td>'
                + '<td><button class="btn-del" onclick="deleteBlog(' + b.id + ')"><i class="fas fa-trash"></i></button></td>'
                + '</tr>';
        }).join('');

    // Categories table
    var catLangs = ['en','fr','es','it','de','zh'];
    document.getElementById('cat-table').innerHTML =
        '<tr><th>#</th><th>القسم</th><th>ترجمة</th><th>حذف</th></tr>' +
        db.categories.map(function(cat, i) {
            var translated = catLangs.filter(function(l){
                return LANGS[l] && LANGS[l].catNames && LANGS[l].catNames[cat] && LANGS[l].catNames[cat] !== cat;
            }).length;
            var badge = translated >= 6
                ? '<span style="background:#22c55e;color:#fff;border-radius:5px;padding:2px 7px;font-size:.68rem;">🌍 6</span>'
                : translated > 0
                    ? '<span style="background:#f59e0b;color:#fff;border-radius:5px;padding:2px 7px;font-size:.68rem;">🔄 ' + translated + '/6</span>'
                    : '<span style="background:#e5e7eb;color:#555;border-radius:5px;padding:2px 7px;font-size:.68rem;">—</span>';
            return '<tr>'
                + '<td>' + (i+1) + '</td>'
                + '<td style="font-weight:800;">' + sanitize(cat) + '</td>'
                + '<td>' + badge + ' <button data-cat="' + sanitize(cat) + '" onclick="retranslateCategory(this.dataset.cat)" style="background:var(--primary-blue);color:#fff;border:none;border-radius:5px;padding:2px 8px;cursor:pointer;font-size:.68rem;">↺</button></td>'
                + '<td style="white-space:nowrap;">'
                    + '<button onclick="editCategoryName(' + i + ')" style="background:#f59e0b;color:#fff;border:none;border-radius:5px;padding:3px 7px;cursor:pointer;font-size:.7rem;margin-left:3px;"><i class="fas fa-edit"></i></button>'
                    + '<button onclick="moveCategoryUp(' + i + ')" style="background:#6c757d;color:#fff;border:none;border-radius:5px;padding:3px 7px;cursor:pointer;font-size:.7rem;margin-left:3px;">▲</button>'
                    + '<button onclick="moveCategoryDown(' + i + ')" style="background:#6c757d;color:#fff;border:none;border-radius:5px;padding:3px 7px;cursor:pointer;font-size:.7rem;margin-left:3px;">▼</button>'
                    + '<button class="btn-del" onclick="deleteCategory(' + i + ')"><i class="fas fa-trash"></i></button>'
                + '</td>'
                + '</tr>';
        }).join('');
    ...
    if (catGrid) {
        ...
        if (!cats.length) {
            catGrid.innerHTML = '<div style="grid-column:1/-1;text-align:center;padding:30px;color:var(--adm-muted,#6b7a99);"><i class="fas fa-tags" style="font-size:2rem;margin-bottom:10px;display:block;"></i>لا توجد أقسام — أضف قسماً أعلاه</div>';
        } else {
            var CAT_ICON_MAP = {
                'Kitchen Finds':'fas fa-utensils','Home Finds':'fas fa-home',
                'آثاث':'fas fa-couch','إلكترونيات':'fas fa-laptop',
                'اكسسوارات إلكترونيات':'fas fa-plug','صحة وتجميل':'fas fa-heart',
                'ملابس رجالي':'fas fa-tshirt','ملابس حريمي':'fas fa-female',
                'ملابس أطفال بناتي':'fas fa-child','ملابس أطفال ولادي':'fas fa-child',
                'ساعات':'fas fa-clock','مجوهرات وإكسسوارات':'fas fa-gem',
                'حقائب':'fas fa-shopping-bag','طبي ودواء':'fas fa-pills'
            };
            var GRAD_COLORS = [
                'linear-gradient(135deg,#0a1628,#1a3a5c)',
                'linear-gradient(135deg,#0d1f10,#1a4020)',
                'linear-gradient(135deg,#1a0e03,#3d2008)',
                'linear-gradient(135deg,#0d0520,#1e0a45)',
                'linear-gradient(135deg,#1a0303,#3d0808)',
                'linear-gradient(135deg,#03111a,#072838)',
                'linear-gradient(135deg,#0a1628,#0d1f55)',
                'linear-gradient(135deg,#1a1003,#3d2800)',
            ];
            catGrid.innerHTML = cats.map(function(cat, i) {
                var icon = CAT_ICON_MAP[cat] || 'fas fa-tag';
                var prod_count = (db.products || []).filter(function(p){ return p.category === cat; }).length;
                var grad = GRAD_COLORS[i % GRAD_COLORS.length];
                return '<div class="cat-admin-card" style="background:'+grad+';border:1px solid rgba(255,255,255,.08);border-radius:14px;padding:16px 14px;position:relative;cursor:grab;">'
                    + '<div style="display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:10px;">'
                    +   '<i class="'+sanitize(icon)+'" style="font-size:1.4rem;color:rgba(255,255,255,.7);"></i>'
                    +   '<div style="display:flex;gap:4px;">'
                    +     '<button onclick="moveCategoryUp('+i+')" title="أعلى" style="background:rgba(255,255,255,.08);border:none;color:#fff;border-radius:6px;width:26px;height:26px;cursor:pointer;font-size:.75rem;" '+(i===0?'disabled style="opacity:.3;cursor:not-allowed;"':'')+'><i class="fas fa-arrow-up"></i></button>'
                    +     '<button onclick="moveCategoryDown('+i+')" title="أسفل" style="background:rgba(255,255,255,.08);border:none;color:#fff;border-radius:6px;width:26px;height:26px;cursor:pointer;font-size:.75rem;" '+(i===cats.length-1?'disabled style="opacity:.3;cursor:not-allowed;"':'')+'><i class="fas fa-arrow-down"></i></button>'
                    +     '<button onclick="deleteCategory('+i+')" title="حذف" style="background:rgba(239,68,68,.2);border:none;color:#fca5a5;border-radius:6px;width:26px;height:26px;cursor:pointer;font-size:.75rem;"><i class="fas fa-trash"></i></button>'
                    +   '</div>'
                    + '</div>'
                    + '<div style="font-weight:800;font-size:.88rem;color:#fff;margin-bottom:4px;">'+sanitize(cat)+'</div>'
                    + '<div style="font-size:.72rem;color:rgba(255,255,255,.45);">'+prod_count+' منتج</div>'
                    + '</div>';
            }).join('');
        }
    }
    // Also update cat-table (compat)
